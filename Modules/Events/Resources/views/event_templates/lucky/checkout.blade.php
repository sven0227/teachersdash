        ...
        <div class="row">
            <div class="col-md-6 order-md-2 mb-4">
                ...
                <ul class="list-group mb-3 sticky-top">
                    ...
                    @if (session()->has("discount_amount"))
                        @php $discount_amount = session()->get("discount_amount") @endphp
                        <li class="list-group-item d-flex justify-content-between lh-condensed">
                            <h6 class="my-0">Discount Amount</h6>
                            <span class="text-muted">{{ $discount_amount }} USD</span>
                        </li>
                    @endif
            ...
            <div class="col-md-6 order-md-1">
                ...
                <form id="form-checkout-event" class="needs-validation" novalidate="">
                    <div class="mb-3">
                        <label for="username">@lang('Full Name')*</label>
...
{{--                        </div>--}}
                        ...
                        @if(getValueIfKeyIsset($user->settings,'IS_STRIPE') || !getValueIfKeyIsset($user->settings,'IS_PAYPAL'))
                            <button id="submit" class="btn btn-success btn-lg btn-block mt-3" style="padding: 7px;" type="submit">
                                <div class="spinner hidden" id="spinner"></div>
                                <span id="button-text">@lang('Stripe')</span>
                            </button>
                        @endif
                    </div>
...
@if(getValueIfKeyIsset($user->settings, 'IS_PAYPAL'))
<div id="paypal-button-container"></div>
<!-- Sample PayPal credentials (client-id) are included -->
<script src="https://www.paypal.com/sdk/js?client-id={{getValueIfKeyIsset($user->settings, 'PAYPAL_CLIENT_ID')}}&currency=USD&intent=capture" data-sdk-integration-source="integrationbuilder"></script>
<script>
...
<script>
    let totalPrice = {{$total_price}};
        const stripe = Stripe("{{getValueIfKeyIsset($user->settings, 'STRIPE_KEY')}}");
