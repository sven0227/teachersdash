...
<li class="nav-item {{ (request()->is('events*')) ? 'active' : '' }}" data-target="event-tour">
    ...
    <div id="collapseEvents" class="collapse {{ (request()->is('events*')) ? 'show' : '' }}"
        ...
        <div class="py-2 collapse-inner rounded">
            ...
            <a class="collapse-item expired-events-link {{ routeName() == 'events.expired' ? 'active' : '' }}"
               href="{{ route('events.expired') }}">@lang('Expired events')</a>
...
<li class="nav-item {{ request()->is('templates*') ? 'active' : '' }}">
    <a class="nav-link" href="{{ route('templates.index') }}">
        <i class="fas fa-th-large"></i> @lang('Templates')</a>
</li>
